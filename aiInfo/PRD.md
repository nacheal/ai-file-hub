...
- 问答基于 RAG 架构：对问题向量化，检索最相关的 top-5 文本块注入 Prompt（替代原始全文注入方案）
- 回答以流式方式逐字输出（Streaming）
- 每次问答历史记录保留在当前会话中（刷新后清空，不持久化）
- 检索无结果时（相似度过低），回复「文档中未找到相关内容」

**F-13 文本分块与向量存储**
- 文件上传并经 AI 分析提取文本后，自动执行分块（chunking）与向量化（embedding）
- 分块规则：目标 500 字符/块，相邻块重叠 50 字符，优先在段落（`\n\n`）处断开，最小块 100 字符
- 每个 chunk 独立生成 1536 维向量，存入 `document_chunks` 表
- 图片文件无 full_text，跳过分块流程
- 分块失败不影响文件分析的主流程状态

**F-14 跨文件全局问答**
- Dashboard 页面顶部提供「问所有文件」入口
- 用户可针对自己所有文件发起全局提问，检索范围为当前用户全部 chunks（top-8）
- 回答中标注每段引用来源的文件名，支持点击跳转对应文件详情页
- 交互形式与单文件问答一致（流式输出 + 多轮历史）
...
| 模型 | `deepseek-chat`（问答与分析）；`deepseek-embedding`（文本向量化，1536 维） |
| Embedding 接口 | `https://api.deepseek.com/v1/embeddings`，OpenAI 兼容，复用 `DEEPSEEK_API_KEY` |
...
-- 不创建 INSERT policy，仅 Service Role（Edge Function）可写
```
...
| `/dashboard` | 主页（文件列表） | 登录后默认页，展示所有文件 + 跨文件全局问答入口 |
| `/file/:id` | 文件详情页 | AI 分析结果 + 单文件 RAG 问答 |
...
| SearchBar | 顶部全局搜索框 |
| GlobalChatPanel | Dashboard 全局问答面板，含来源文件卡片（SourceCard） |
...
| 阶段 | 内容 | 状态 |
|------|------|------|
| 阶段一：地基 | Supabase 项目初始化、建表、GitHub OAuth、Vite + React 初始化、Vercel 部署 | ✅ 已完成 |
| 阶段二：上传管道 | 文件上传 UI、Storage 接入、元数据写库、Realtime 订阅、状态实时更新 | ✅ 已完成 |
| 阶段三：AI 分析 | Edge Function 编写、DeepSeek API 接入、文件解析、分析结果存库与展示 | ✅ 已完成 |
| 阶段四：搜索与问答 | 全文搜索、文件详情页、AI 流式问答 | ✅ 已完成 |
| 阶段五：打磨上线 | UI 美化（shadcn/ui）、错误处理、RLS 安全检查、移动端适配 | ✅ 已完成 |
| 阶段六：RAG 升级 | pgvector + 分块向量存储、RAG 单文件问答、跨文件全局问答 | ✅ 已完成 |
...
- [ ] `document_chunks` 表创建成功，上传文件后有对应 chunk 记录且 embedding 非 null
- [ ] 单文件问答基于 RAG 检索，大文件（> 8000 字符）不再因截断缺失答案
- [ ] 跨文件全局问答可返回来自不同文件的内容，并正确标注来源文件名
- [ ] 删除文件后对应 `document_chunks` 记录自动级联删除
